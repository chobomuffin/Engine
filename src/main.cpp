...
void CoolGame::update(int delta)
{
  ...
  if (accumulator >= (1000.0 / frameRate)) {
    ...
    textureData->updateTexture(pCodecCtx->width, pCodecCtx->height, pFrameRGB->data[0], GL_TEXTURE_2D, GL_LINEAR);
...
void CoolGame::init(GLManager *glManager)
{
  ...
  if (avformat_open_input(&pFormatCtx, "/Users/danielw/workspace/Engine/assets/movie 01.mov", NULL, NULL) != 0)
    exit(1);
  ...
  ///////////////////  ///////////////////  ///////////////////  ///////////////////
  textureData = new TextureData(pCodecCtx->width, pCodecCtx->height, pFrameRGB->data[0], GL_TEXTURE_2D, GL_LINEAR);
  Texture* texture = new Texture(textureData);

  ImageLoader il = ImageLoader(texture);
